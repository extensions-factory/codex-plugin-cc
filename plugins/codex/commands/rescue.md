...
argument-hint: "[--background|--wait] [--resume|--fresh] [--worker <name>] [--model <model|spark>] [--effort <none|minimal|low|medium|high|xhigh>] [what Codex should investigate, solve, or continue]"
...
- `--worker`, `--model`, and `--effort` are runtime-selection flags. Preserve them for the forwarded `task` call, but do not treat them as part of the natural-language task text.
...
- Leave `--worker` unset unless the user names a worker or asks for a role the roster covers. The runtime applies its default worker, which supplies the model and the worker instructions.
